...
int scan_complete(char** args,char** pargs ,int* pt)
{
    int j=0,i=*pt;
    while(1)
    {
...
void execute(char** args)
{
    ...
    if(!fork())
    {
        while(1)
        {
            //printf("stuck :(\n");
            //printf("at %d\n",pt);
            if(scan_complete(args,pargs,&pt)==0)
            {
                //fprintf(stderr, "pt = %d\n",pt);
                pipe(p);
                if(!fork())
